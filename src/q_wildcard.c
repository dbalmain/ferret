...
static Query *wcq_rewrite(Query *self, IndexReader *ir)
{
    ...
    const char *field = WCQ(self)->field;
    const char *pattern = WCQ(self)->pattern;
    const char *first_star = strrchr(pattern, WILD_STRING);
    const char *first_ques = strrchr(pattern, WILD_CHAR);
    ...
    if (!first_star && !first_ques) {
        ...
        if (field_num >= 0) {
            ...
            pattern = (first_ques && (first_star > first_ques))
...
static f_u32 wcq_hash(Query *self)
{
