...
class FSStoreTest < Test::Unit::TestCase
  ...
  def test_fslock
    ...
    assert(! File.exist?(lock_file_path), "There should be no lock file")
    lock = @dir.make_lock(lock_name)
    assert(! File.exist?(lock_file_path), "There should still be no lock file")
    ...
    assert(File.exist?(lock_file_path),   "A lock file should have been created")
    ...
    assert(! File.exist?(lock_file_path), "The lock file should have been deleted")
  end
...
#    make_and_loose_lock
#
#    #assert(File.exist?(lock_file_path), "There should now be a lock file")
#
#    lock = @dir.make_lock(lock_name)
#    assert(lock.locked?, "lock should now be locked")
#
...
#
  ...
  def make_lock_file_path(name)
    ...
    if File.exist?(lock_file_path) then
