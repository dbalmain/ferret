...
module Ferret
  module Index
    ...
    class IndexWriter
      ...
      attr_reader :analyzer, :directory, :merge_factor
      ...
      def initialize(dir, analyzer, create, close_dir = true)
        super()

        if dir.instance_of?(String)
          @directory = FSDirectory.get_directory(dir, create)
        else
          @directory = dir
        end
        @close_dir = close_dir
        @analyzer = analyzer || StandardAnalyzer.new
      ...
      private
        ...
        def merge_segments(min_segment, max_segment = @segment_infos.size)
          ...
          merged_name = new_segment_name()
          if @info_stream != nil
            @info_stream.print("merging segments from " + min_segment.to_s +
                               " to " + (max_segment - 1).to_s + "\n")
          end
          merger = SegmentMerger.new(@directory, merged_name, @term_index_interval)
          ...
          (min_segment ... max_segment).each do |i|
            si = @segment_infos[i]
            if (@info_stream != nil)
              @info_stream.print(" " + si.name + " (" + si.doc_count.to_s + " docs)\n")
            end
