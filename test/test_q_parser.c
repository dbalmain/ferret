...
{
    int i;
    HashSet *all_fields = hs_str_new(NULL);
    HashSet *def_fields = hs_str_new(NULL);
    ...
    REF(analyzer);
    parser = qp_new(all_fields, def_fields, analyzer);
...
static void test_qp_bad_queries(tst_case *tc, void *data)
{
    ...
    parser = qp_new(all_fields, def_fields, letter_analyzer_new(true));
